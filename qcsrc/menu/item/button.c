...
CLASS(Button) EXTENDS(Label)
	...
	ATTRIB(Button, onRightClick, void(entity, entity), SUB_Null)
	// used for left and right clicks..
	...
	ATTRIB(Button, leftClick, float, true)
...
float mouseReleaseButton(entity me, vector pos)
{
	...
	if(me.pressed)
	{
		// BlackHC trigger it at only place only (below)
		me.clickTime = 0.001;
		me.pressed = 0;
	}
	return 1;
	...
	if(me.clickTime > 0 && me.clickTime <= frametime)
	{
		...
		if not(me.disabled)
			if( me.leftClick )
				...
				me.onRightClick(me, me.onClickEntity);
