...
CLASS(Nexposee) EXTENDS(Container)
	...
	ATTRIB(Nexposee, animationState, float, -2)
...
void resizeNotifyNexposee(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	...
	entity e;
	// Not on the first resizeNotify!
	if( me.animationState >= -1 ) {
		for(e = me.firstChild; e; e = e.nextSibling) {
			e.Container_size = e.Nexposee_initialSize;
			e.Container_origin = e.Nexposee_initialOrigin;
		}
	} else {
		me.animationState = -1;
	}
	...
	for(e = me.firstChild; e; e = e.nextSibling) {
		e.Nexposee_initialSize = e.Container_size;
		e.Nexposee_initialOrigin = e.Container_origin;
	}
}
